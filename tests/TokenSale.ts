...
import { MyERC721Token__factory } from "../typechain-types";
...
describe("NFT Shop", async () => {
  ...
  describe("When a user purchase an ERC20 from the Token contract", async () => {
    const ETH_SENT = ethers.utils.parseEther("0.0001");
    ...
    describe("When a user burns an ERC20 at the Shop contract", async () => {
      ...
      it("gives the correct amount of ETH", async () => {
        ...
        const expectedBalance = balanceBefore.sub(gasCost).add(ETH_SENT);
        expect(balanceAfterBurn).to.eq(expectedBalance);
      });
